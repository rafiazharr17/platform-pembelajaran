<x-app-layout>
    ...
    <div class="mt-8 px-4 sm:px-6 lg:px-8">
        ...
        {{-- Form Pencarian --}}
        <div class="mb-6">
            <form method="GET" action="{{ route('materi.index') }}" class="flex flex-col sm:flex-row sm:items-center gap-2 max-w-full sm:max-w-xl">
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($materi as $item)
                    <div class="bg-white p-6 rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition duration-300 flex flex-col justify-between">
                        {{-- Preview File --}}
                        @if ($item->file_path)
                            ...
                            @elseif ($item->file_type === 'video')
                                @if ($item->thumbnail_path)
                                    <img src="{{ asset('storage/' . $item->thumbnail_path) }}" alt="Thumbnail Video"
                                        class="mb-4 w-full h-40 object-cover rounded-lg border border-blue-200 shadow">
                                @else
                                    <div class="mb-4 flex items-center justify-center h-40 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        {{-- Tombol Aksi --}}
                        <div class="flex flex-wrap justify-between items-center text-sm mt-auto gap-2">
